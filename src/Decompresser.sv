...
assign r_end_req = (decompress_state == COMPLETE) || (decompress_state == IDLE);
assign w_end_req = (decompress_state == COMPLETE) || (decompress_state == IDLE); //w_end_req를 주면 w_state가 마지막 픽셀들을 Decompressed FIFO에 쓰고 IDLE로 가므로 계속 신호를 주고있어야 함. 
...
always @(*) begin
    ...
    case(decompress_state)
        ...
        COMPLETE: begin
            ...
            //w_end_req = 1;
            decompress_state_next = IDLE; //바로 IDLE로 이동.
        end
    endcase
end
...
always @(*) begin
    ...
    Decompressed_FIFO_data[63:0] = 0; //decompressed_fifo_r_state에 의해서 결정됨. output으로 나가서 외부에 주는 값임.
    ...
    Compressed_FIFO_dequeue = 0; //Compressed_FIFO_ena는 r_state에서 결정함. 여기서는 dequeue만 설정함.
    ...
    compressed_fifo_r_data[63:0] = 0; //이 신호를 compressed_fifo_r_ena가 오면 r_state에 주는거임.

    //compressed_fifo_r_end와 decompressed_fifo_w_end는 따로 assign문으로 독립시켰음.
