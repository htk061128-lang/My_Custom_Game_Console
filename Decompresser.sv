...
wire tdm_match = (BRAM_rw_counter_bits[0] && Clk_Counter[3:0] == 0) || (BRAM_rw_counter_bits[1] && Clk_Counter[3:0] == 1) || (BRAM_rw_counter_bits[2] && Clk_Counter[3:0] == 2) || (BRAM_rw_counter_bits[3] && Clk_Counter[3:0] == 3);
...
parameter IDLE = 0, START = 1, COMPLETE = 2, PIXEL_REPEAT = 3, PIXEL_UNREPEAT = 4, SAME_PIXEL = 5, WAIT_DATA = 6, READ = 7, DELAY = 8;
...
reg decompressed_FIFO_reg_wstrb[7:0]; //8개 비트중에서 1로 설정된 위치에 w_8를 쓰는 제어신호.
...
always @(*) begin
    ...
    decompressed_FIFO_reg_wstrb[7:0] = 8'b0;
    ...
    case(r_state)
        ...
        COMPLETE: begin
            Decompressed_FIFO_ena = 0; //0으로 쭉 내려줘서, Compressed_Data_FIFO 모듈에서 외부 메모리에서 데이터읽기요청을 중단시킴.
    ...
    case(w_state)
        ...
        START: begin //decompressed_FIFO_reg_counter 가 0으로 초기화되어 있어야 함!!!!
            ...
            endcase
            if(w_end_req) begin
                w_state_next = COMPLETE; //w_end_req를 받으면 즉시 COMPLETE로 이동해야 함.
            end
        end
        DELAY: begin
            ...
            else if(decompressed_fifo_w_end) begin
                w_state_next = START;
...
always @(posedge clk or negedge resetn) begin
    if(!resetn) begin
        ...
        if(r_state_next == IDLE && r_state == COMPLETE) begin //레지스터들 다 초기화 해줘야 함.
        end
        if(decompress_state_next == IDLE && decompress_state == COMPLETE) begin
        end
        if(w_state_next == IDLE && w_state == COMPLETE) begin
        end
