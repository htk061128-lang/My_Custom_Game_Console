...
always @(*) begin
    ...
    case(main_state) //main_state가 Pixel_Processer에게 RGB, valid를 보내고 ready를 받는 핸드쉐이크를 전담함.
        ...
        NO_BG_START: begin
            main_state_next = (personal_counter_x == 319 && personal_counter_y == 239 && Pixel_valid && Pixel_ready) ? IDLE : NO_BG_START;
            main_state_counter_next[3:0] = 0;
...
always @(posedge clk or negedge resetn) begin
    if(!resetn) begin
        ...
        case(rgb_r_state)
            ...
            START: begin
                if(fifo_r_state == IDLE) begin
                    rgb_r_state <= IDLE;
                    rgb_r_state_counter[3:0] <= 0;
                end
                else if(rgb_convert_req) begin
                    rgb_r_state <= READ;
                    rgb_r_state_counter[3:0] <= 0;
                end
                else begin
